...
public class FollowCommand: InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task StartTrackingAsync(string blueskyUser, ITextChannel? channel = null)
    {
        ...
        var channelDbObject = await dbContext.DiscordChannels.Include(discordChannel => discordChannel.TrackedUsers).FirstOrDefaultAsync(it => it.ChannelId == targetChannel.Id) ?? new DiscordChannel
        {
            ChannelId = targetChannel.Id,
            ServerId = targetChannel.GuildId,
        };
        if(channelDbObject.TrackedUsers.Any(u => u.DidObject == result.Did))
        {
