...
public class InviteCommand : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task GenerateInviteLink()
    {
        ...
        await RespondAsync($"https://discord.com/api/oauth2/authorize?client_id={Context.Client.CurrentUser.Id}&permissions={(ulong) requiredPermissions.Permission}&scope={string.Join("%20", requiredPermissions.Scopes)}", ephemeral: true);
    }
}
